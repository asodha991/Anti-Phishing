...
def to_find_authority(url):
  ...
  cmd = "curl" + url
  # stdout = Popen( ['curl', url] , shell=True, stderr=PIPE, env={}).stderr

  
  stdout= Popen(cmd, shell=True, stderr=PIPE).communicate()[1]
  # stdout = Popen(cmd, shell=True, stderr = PIPE, stdin=PIPE,stdout=PIPE, env={} ) 
  # stdout = stdout.s
  # print(stdout)
  # output = stdout.read()
  # print(output)
...
#   # features_extracted[18] = check_iframe(url)
...
# URL = pd.read_csv('/content/URL_Small.csv')
